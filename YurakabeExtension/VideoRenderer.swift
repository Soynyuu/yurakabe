...
final class VideoRenderer: @unchecked Sendable {
    ...
    private let desktopColorLayer = CALayer()
    private var asset: AVURLAsset
    private var videoTrack: AVAssetTrack
    private var encodedSize: CGSize
    private var clipDuration: CMTime = .zero
    ...
    static func create(
        ...
        displayLayer.frame = VideoFit.rect(video: encodedSize, inside: rootLayer.bounds)
    ...
    private init(
        ...
        stillFrameLayer.name = "yurakabe.stillFrame"
        ...
        rootLayer.sublayers?.filter { $0.name == "yurakabe.stillFrame" }.forEach { $0.removeFromSuperlayer() }
        rootLayer.backgroundColor = CGColor(gray: 0, alpha: 1)
        rootLayer.masksToBounds = true
        rootLayer.addSublayer(displayLayer)
        extensionLog("[VideoFit #\(debugID)] encoded=\(encodedSize), bounds=\(rootLayer.bounds), frame=\(displayLayer.frame), gravity=resize")
        rootLayer.addSublayer(stillFrameLayer)
        desktopColorLayer.frame = rootLayer.bounds
        desktopColorLayer.backgroundColor = DesktopAppearance.color
        desktopColorLayer.isOpaque = true
        desktopColorLayer.backgroundColor = DesktopAppearance.color
        desktopColorLayer.isHidden = DesktopAppearance.showsVideo
        desktopColorLayer.name = "lock-only.desktopColor"
        rootLayer.addSublayer(desktopColorLayer)
    ...
    func resize(to destSize: CGSize, scale: CGFloat) {
        ...
        CATransaction.begin()
        CATransaction.setDisableActions(true)
        displayLayer.frame = VideoFit.rect(video: encodedSize, inside: bounds)
        displayLayer.contentsScale = scale
        stillFrameLayer.frame = bounds
        desktopColorLayer.frame = bounds
    ...
    func switchVideo(to url: URL) {
        ...
        queue.async { [weak self] in
            ...
            asset = newAsset
            videoTrack = track
            let geometry = Self.loadGeometryBlocking(newAsset.url)
            encodedSize = geometry.size
            clipDuration = geometry.duration
            CATransaction.begin()
            CATransaction.setDisableActions(true)
            displayLayer.frame = VideoFit.rect(video: encodedSize, inside: desktopColorLayer.frame)
            CATransaction.commit()
            CATransaction.flush()
    ...
    /// Only called on the owned renderer queue, never the cooperative executor.
    private static func loadGeometryBlocking(_ url: URL) -> (size: CGSize, duration: CMTime) {
        let semaphore = DispatchSemaphore(value: 0)
        nonisolated(unsafe) var result: (CGSize, CMTime) = (.zero, .zero)
        Task.detached {
    ...
    func stop() {
        ...
        desktopColorLayer.removeFromSuperlayer()
    }
    ...
    func applyPolicy(_ policy: PlaybackPolicy, animated: Bool = false) {
        ...
        CATransaction.commit()
        CATransaction.flush()
        let policy: PlaybackPolicy = DesktopAppearance.showsVideo ? policy : .paused
        // Apply mode changes immediately; do not fade video onto a static desktop.
        ...
        switch policy {
        case .paused: pause()
        case .full, .reduced, .minimal: resume()
        }
    }
    ...
    private static let deepPauseDelay: TimeInterval = 1
    ...
    ///   left off, without restarting from the beginning.
    ...
    private func recreatePlayback(seamlessResume: Bool = false) {
        ...
        if !clipDuration.isNumeric || clipDuration <= .zero {
            clipDuration = Self.loadGeometryBlocking(asset.url).duration
        }
        let resumeTime = LoopPosition.withinClip(CMTimebaseGetTime(timebase), duration: clipDuration)
        ...
        CMTimebaseSetTime(timebase, time: continuing ? resumeTime : .zero)
    ...
    private func swapToNextReader() {
        ...
        extensionLog("[Yurakabe loop #\(debugID)] boundary at \(ptsOffset.seconds)s, mode=\(WallpaperState.shared.presentationMode)")

...
func writeBMPSnapshot(videoURL: URL, videoID: String? = nil, displayPixelWidth: Int, displayPixelHeight: Int) async {
    ...
    // A cached desktop must never display a frozen video frame during transitions.
    guard let cgImage = DesktopAppearance.snapshot(width: displayPixelWidth, height: displayPixelHeight) else { return }
